...
countMovement :: [[Char]] -> Int
countMovement lineList = length [x | x <- concat lineList , x=='X']
...
countSteps :: [[Char]] -> (Int, Int) -> ([[Char]], (Int, Int))
countSteps lineList (x, y)
    | nextSquare == '#' = changeThingOnGrid (x, y) nextSym lineList), (x, y)
    | nextSquare == '.' = changeThingOnGrid nextPos sym removedGuard), nextPos
    | nextSquare == 'X' = changeThingOnGrid nextPos sym removedGuard), nextPos
    | otherwise = 0
    where
